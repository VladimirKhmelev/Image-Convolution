...
CSV_DIR="benchmark_data/strategies"
mkdir -p "$CSV_DIR" plots/strategies
...
for IMG in "${IMAGES[@]}"; do
    ...
    echo ""
    echo "=== $IMG ==="
    ./gradlew -q run --args="$IMG \"Gaussian Blur 3×3\" --benchmark --csv $CSV"
    echo "CSV → $CSV"
done

echo ""
echo "=== Строю графики ==="
python3 scripts/plot.py "${CSV_FILES[@]}" --out plots/strategies

echo ""
echo "Готово. Графики в plots/strategies/"
